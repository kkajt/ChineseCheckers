package pl.kkorzycki.chinesecheckers.Board;

import org.junit.Assert;
import org.junit.Before;
import org.junit.Test;
import pl.kkorzycki.chinesecheckers.Colour;
    ...
    public void testIfPawnsAreWellPlaced() {
        ...
        for (int i = 1; i <= 4; i++) {
            for (int j = 0; j < twoPlayerBoard.getMap()[i].length; j++) {
                if (board.getMap()[i][j] != null) {
                    Assert.assertEquals(testPawn1, twoPlayerBoard.getMap()[i][j].getPawn());
                    Assert.assertEquals(testPawn1, threePlayerBoard.getMap()[i][j].getPawn());
                    Assert.assertEquals(testPawn1, fourPlayerBoard.getMap()[i][j].getPawn());
                    Assert.assertEquals(testPawn1, sixPlayerBoard.getMap()[i][j].getPawn());
                }
            }
        }
        ...
        for (int i = 5; i <= 8; i++) {
            for (int j = position; j < twoPlayerBoard.getMap()[i].length; j++) {
                if (board.getMap()[i][j] != null) {
                    Assert.assertEquals(null, twoPlayerBoard.getMap()[i][j].getPawn());
                    Assert.assertEquals(null, fourPlayerBoard.getMap()[i][j].getPawn());
                    Assert.assertEquals(null, threePlayerBoard.getMap()[i][j].getPawn());
                    Assert.assertEquals(testPawn2, sixPlayerBoard.getMap()[i][j].getPawn());
                }
            }
            position++;
        }
        ...
        for (int i = 10; i <= 13; i++) {
            for (int j = position; j < twoPlayerBoard.getMap()[i].length; j++) {
                if (board.getMap()[i][j] != null) {
                    Assert.assertEquals(null, twoPlayerBoard.getMap()[i][j].getPawn());
                    Assert.assertEquals(testPawn3, threePlayerBoard.getMap()[i][j].getPawn());
                    Assert.assertEquals(testPawn3, fourPlayerBoard.getMap()[i][j].getPawn());
                    Assert.assertEquals(testPawn3, sixPlayerBoard.getMap()[i][j].getPawn());
                }
            }
            position--;
        }
        ...
        for (int i = 14; i <= 17; i++) {
            for (int j = 0; j < twoPlayerBoard.getMap()[i].length; j++) {
                if (board.getMap()[i][j] != null) {
                    Assert.assertEquals(testPawn4, twoPlayerBoard.getMap()[i][j].getPawn());
                    Assert.assertEquals(null, threePlayerBoard.getMap()[i][j].getPawn());
                    Assert.assertEquals(testPawn4, fourPlayerBoard.getMap()[i][j].getPawn());
                    Assert.assertEquals(testPawn4, sixPlayerBoard.getMap()[i][j].getPawn());
                }
            }
        }
        ...
        for (int i = 13; i >= 10; i--) {
            for (int j = position; j >= 0; j--) {
                if (board.getMap()[i][j] != null) {
                    Assert.assertEquals(null, twoPlayerBoard.getMap()[i][j].getPawn());
                    Assert.assertEquals(testPawn5, threePlayerBoard.getMap()[i][j].getPawn());
                    Assert.assertEquals(null, fourPlayerBoard.getMap()[i][j].getPawn());
                    Assert.assertEquals(testPawn5, sixPlayerBoard.getMap()[i][j].getPawn());
                }
            }
            position--;
        }
        ...
        for (int i = 8; i >= 5; i--) {
            for (int j = position; j >= 0; j--) {
                if (board.getMap()[i][j] != null) {
                    Assert.assertEquals(null, twoPlayerBoard.getMap()[i][j].getPawn());
                    Assert.assertEquals(null, threePlayerBoard.getMap()[i][j].getPawn());
                    Assert.assertEquals(testPawn6, fourPlayerBoard.getMap()[i][j].getPawn());
                    Assert.assertEquals(testPawn6, sixPlayerBoard.getMap()[i][j].getPawn());
                }
            }
            position++;
        }
